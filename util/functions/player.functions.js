...
exports.getMostExpensiveDeath = (history) => {
  ...
  history.forEach((e) => {
    ...
    if (e.Victim.DeathFame > player.DeathFame) {
      (player = e.Victim), (participants = e.Participants);
    }
  });
...
exports.getHighestAssists = ({ players }) => {
  ...
  players.forEach((p) => {
    ...
    if (
      p.totalKillContribution === highest[0].totalKillContribution &&
      p.name !== highest[0].name
    ) {
      highest.push(p);
    }
  });
...
exports.getHighestHealingPlayer = ({ players }) => {
  ...
  players.forEach((p) => {
    ...
    if (
      p.totalHealing === player[0].totalHealing &&
      p.name !== player[0].name
    ) {
      player.push(p);
    }
  });
  ...
  playerIds.forEach((p) => {
    ...
    playerInfo.forEach((f) => {
      if (f.Name === player.name) {
        ...
        player.role = assignPlayerRoles(player);
      }
    });
...
const assignPlayerRoles = (player) => {
  if (_.includes(battleMount, player.equipment.mount)) {
    return "battlemount";
  }
  let weapon;
  if (player.equipment.mainhand !== null) {
    if (
      player.equipment.mainhand[player.equipment.mainhand.length - 2] === "@"
    ) {
      weapon = player.equipment.mainhand.slice(0, -2);
    } else {
      weapon = player.equipment.mainhand;
    }
    if (_.includes(support, weapon)) {
      return "support";
    }
    if (_.includes(range, weapon)) {
      return "range";
    }
    if (_.includes(melee, weapon)) {
      return "melee";
    }
    if (_.includes(tank, weapon)) {
      return "tank";
    }
    if (_.includes(flank, weapon)) {
      return "flank";
    }
    if (_.includes(healer, weapon)) {
      return "healer";
    }
  }
  return "unknown";
